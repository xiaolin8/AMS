...
function Loading(bool, text) {
    ...
    if (!!text) {
        ...
        top.$("#loading span").html("正在努力为您加载…");
    }
    if (bool) {
        ajaxbg.show();
    } else {
...
function PartialButton() {
    getAjax("/Utility/LoadButton", {}, function (data) {
        $('.tools_bar .PartialButton').html(data);
    });
}
function PartialButton2() {
    var JsonData = "";
    var Date = "";
    var toolbar_html = "";
    var toolbar_Children_Count = 0;
    AjaxJson("/Utility/LoadButton", {}, function (data) {
        Data = new Function("return (" + data + ")")().Date; // eval("(" + data + ")");
        JsonData = Data;
        $.each(Data, function (i) {
            if (Data[i].Category == "工具栏") {
                ...
                if (toolbar_Children_Count > 0) {
                    ...
                    toolbar_html += "<div style=\"float: left;\"><div class=\"icon\"><img src=\"../../Themes/Images/Icon16/" + Data[i].Img + "\" /></div><div class=\"text\">" + Data[i].FullName + "</div></div>";
                    toolbar_html += "<div class=\"dropdown-icon\"><img src=\"../../Themes/Images/dropdown-icon.png\" /></div>";
                    ...
                    var title = "title=\"" + Data[i].Description + "\"";
                    if (Data[i].Description == "") { title = ""; }
                    toolbar_html += "<a id=\"" + Data[i].Code + "\" " + title + " onclick=\"" + Data[i].JsEvent + "\" class=\"tools_btn\"><span><b style=\"background: url('../../Themes/Images/Icon16/" + Data[i].Img + "') 50% 4px no-repeat;\">" + Data[i].FullName + "</b></span></a>";
                }
                if (Data[i].Split == '1') {
    ...
    function Toolbar_Children(ButtonId, Data) {
        ...
        $.each(Data, function (i) {
            if (Data[i].MenuId == ButtonId) {
                var title = "title=\"" + Data[i].Description + "\"";
                if (Data[i].Description == "") { title = ""; }
                _toolbar_Children += "<li id=\"" + Data[i].Code + "\" " + title + " onclick=\"" + Data[i].JsEvent + "\"><img src=\"../../Themes/Images/Icon16/" + Data[i].Img + "\" />" + Data[i].FullName + "</li>";
    ...
    $.each(JsonData, function (i) {
        if (JsonData[i].ParentId == '0' && JsonData[i].Category == '2') {
            ...
            if (right_toolbar_Count > 0) {
                right_toolbar_html += "<li id=\"right_" + JsonData[i].Code + "\" " + title + " onclick=\"" + JsonData[i].JsEvent + "\" ><img src=\"../../Themes/Images/Icon16/" + JsonData[i].Img + "\" />" + JsonData[i].FullName + righttoolbar_Children_List + "</li>";
            } else {
                right_toolbar_html += "<li id=\"right_" + JsonData[i].Code + "\" " + title + " onclick=\"" + JsonData[i].JsEvent + "\" ><img src=\"../../Themes/Images/Icon16/" + JsonData[i].Img + "\" />" + JsonData[i].FullName + "</li>";
            }
    ...
    function right_toolbar_Children(ButtonId, JsonData) {
        ...
        $.each(JsonData, function (i) {
            if (JsonData[i].ParentId == ButtonId) {
                var title = "title=\"" + JsonData[i].Remark + "\"";
                if (JsonData[i].Remark == "") { title = ""; }
                _right_toolbar += "<li id=\"right_" + JsonData[i].Code + "\" " + title + " onclick=\"" + JsonData[i].JsEvent + "\" ><img src=\"../../Themes/Images/Icon16/" + JsonData[i].Img + "\" />" + JsonData[i].FullName + "</li>";
                if (JsonData[i].Split == '1') {
...
function AddTabMenu(tabid, url, name, img, Isclose, IsReplace, IsVisitorModule) {
    ...
    if (top.document.getElementById("tabs_" + tabid) == null) { //如果当前tabid存在直接显示已经打开的tab
        ...
        if (Isclose != 'false') { //判断是否带关闭tab
            tabs_container.append("<li id=\"tabs_" + tabid + "\" class='selected' win_close='true'><span title='" + name + "' onclick=\"AddTabMenu('" + tabid + "','" + url + "','" + name + "','true')\"><a><img src='../Themes/Images/Icon16/" + img + "' width='16' height='16'>" + name + "</a></span><a class=\"win_close\" title=\"关闭当前窗口\" onclick=\"RemoveDiv('" + tabid + "')\"></a></li>");
        } else {
            tabs_container.append("<li id=\"tabs_" + tabid + "\" class=\"selected\" onclick=\"AddTabMenu('" + tabid + "','" + url + "','" + name + "','false')\"><a><img src='../Themes/Images/Icon16/" + img + "' width='16' height='16'>" + name + "</a></li>");
        }
