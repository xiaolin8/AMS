    ...
    top.$.dialog({
        ...
        button: [
        {
            ...
            callback: function () {
                Loading(true, "正在退出系统...");
                window.setTimeout(function () {
                    getAjax("/Home/OutLogin", "", function (data) {
                        window.opener = null;
...
function PersonCenter() {
    var url = "/Home/PersonCenter";
    Dialog(url, "ResetPassword", "个人中心", 750, 400);
}
//快捷方式列表
function ShortcutsList() {
    $("#Shortcuts").html('');
    AjaxJson("/Home/ShortcutsListJson", "", function (data) {
        var DataJson = new Function("return (" + data + ")")(); // eval("(" + data + ")");
        $.each(DataJson, function (i) {
            $("#Shortcuts").append("<li onclick=\"AddTabMenu('" + DataJson[i].ModuleId + "', '" + DataJson[i].Location + "', '" + DataJson[i].FullName + "',  '" + DataJson[i].Icon + "','true');\"><img src=\"/Themes/Images/Icon16/" + DataJson[i].Icon + "\" />" + DataJson[i].FullName + "</li>");
        });
    });
    $(".popup li").click(function () {
        $('.popup').hide();
...
//关于我们
function About() {
    alertDialog("开发生产技术管理系统<br>版本1.0<br>北京侏罗纪软件股份有限公司<br>保留所有权利", 0);
}
//个性化皮肤设置
function SkinIndex() {
